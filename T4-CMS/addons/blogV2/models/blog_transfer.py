...
class BlogTransfer(models.Model):
    ...
    # Tránh tạo nhiều đối tượng không cần thiết: Tiết kiệm bộ nhớ.
    # Đảm bảo tính nhất quán: Dữ liệu và trạng thái được chia sẻ thông qua một instance duy nhất.
    # Ví dụ trong thực tế:
    # Quản lý kết nối cơ sở dữ liệu.
    # Cấu hình hệ thống.
    # Logging.
    @classmethod # cho phép định nghĩa một phương thức ở cấp lớp (class level) thay vì cấp đối tượng (instance level).
        ...
        try:
           ...
           # Chuẩn bị params
            params = {
                ...
                'db_name_local': self.env.cr.dbname # dòng này lấy database hiện tại của localhost
            }
    ...
    def _log_transfer_result(self, post, server, success, message):
        ...
        """
        _logger.info('def _log_transfer_result')
    ...
    def action_start_transfer(self):
        ...
        try:
            ...
            if isSuccess:
                ...
                _logger.info('scheduler: {scheduler}')
